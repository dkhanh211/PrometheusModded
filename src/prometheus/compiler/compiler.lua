...
function Compiler:generateOpMapping()
    local ops = {
        AstKind.AddExpression, AstKind.SubExpression, AstKind.MulExpression, AstKind.DivExpression,
        AstKind.ModExpression, AstKind.PowExpression, AstKind.StrCatExpression,
        AstKind.EqualsExpression, AstKind.NotEqualsExpression, AstKind.LessThanExpression,
        AstKind.GreaterThanExpression, AstKind.LessThanOrEqualsExpression, AstKind.GreaterThanOrEqualsExpression,
        AstKind.NotExpression, AstKind.NegateExpression, AstKind.LenExpression
    }
    self.opMapping = {}
    local ids = {}
    for i = 1, #ops do table.insert(ids, i) end
    util.shuffle(ids)
    for i, op in ipairs(ops) do
        self.opMapping[op] = ids[i]
    end
end

function Compiler:createVmHandler()
    local scope = Scope:new(self.scope)
    local opArg = scope:addVariable()
    local aArg = scope:addVariable()
    local bArg = scope:addVariable()
    
    local elseifs = {}
    local firstOp = nil
    
    local ops = {}
    for kind, id in pairs(self.opMapping) do
        table.insert(ops, {kind = kind, id = id})
    end
    util.shuffle(ops)
    
    for i, op in ipairs(ops) do
        local cond = Ast.EqualsExpression(Ast.VariableExpression(scope, opArg), Ast.NumberExpression(op.id))
        local a = Ast.VariableExpression(scope, aArg)
        local b = Ast.VariableExpression(scope, bArg)
        local res
        
        if op.kind == AstKind.NotExpression then
            res = Ast.NotExpression(a)
        elseif op.kind == AstKind.NegateExpression then
            res = Ast.NegateExpression(a)
        elseif op.kind == AstKind.LenExpression then
            res = Ast.LenExpression(a)
        else
            res = Ast[op.kind](a, b)
        end
        
        local body = Ast.Block({Ast.ReturnStatement({res})}, Scope:new(scope))
        
        if i == 1 then
            firstOp = {condition = cond, body = body}
        else
            table.insert(elseifs, {condition = cond, body = body})
        end
    end
    
    return Ast.FunctionLiteralExpression(
        {Ast.VariableExpression(scope, opArg), Ast.VariableExpression(scope, aArg), Ast.VariableExpression(scope, bArg)},
        Ast.Block({Ast.IfStatement(firstOp.condition, firstOp.body, elseifs, nil)}, scope)
    )
end
...
function Compiler:compile(ast)
    ...
    self.vmHandlerVar = self.scope:addVariable();
    ...
    createClosureScope:addReferenceToHigherScope(self.scope, self.vmHandlerVar);
    ...
    local functionNodeAssignments = {
        {
            ...
            var = Ast.AssignmentVariable(self.scope, self.vmHandlerVar),
            val = self:createVmHandler(),
        },
    }
    ...
    local tbl = {
        ...
        Ast.VariableExpression(self.scope, self.vmHandlerVar),
    };
...
function Compiler:obfuscateString(str)
    local len = #str
    if len <= 1 then
    ...
    local i = 1
    while i <= len do
        local chunk = math.random(1, 3)
        if i + chunk > len + 1 then chunk = len - i + 1 end
        table.insert(parts, Ast.StringExpression(str:sub(i, i + chunk - 1)))
        i = i + chunk
    end
    
    local node = parts[1]
    for j = 2, #parts do
        node = Ast.StrCatExpression(node, parts[j])
    end
    return node
end
    ...
    if(statement.kind == AstKind.FunctionDeclaration) then
        ...
        if(#statement.indices > 0) then
            ...
            if statement.scope.isGlobal then
                ...
                self:addStatement(self:setRegister(scope, tblReg, self:obfuscateString(statement.scope:getVariableName(statement.id))), {tblReg}, {}, false);
            ...
            for i = 1, #statement.indices - 1 do
                ...
                local indexReg = self:compileExpression(self:obfuscateString(index), funcDepth, 1)[1];
        ...
        if statement.scope.isGlobal then
            local tmpReg = self:allocRegister(false);
            self:addStatement(self:setRegister(scope, tmpReg, self:obfuscateString(statement.scope:getVariableName(statement.id))), {tmpReg}, {}, false);
        ...
        for i, primaryExpr in ipairs(statement.lhs) do
            if primaryExpr.kind == AstKind.AssignmentVariable then
                if primaryExpr.scope.isGlobal then
                    local tmpReg = self:allocRegister(false);
                    self:addStatement(self:setRegister(scope, tmpReg, self:obfuscateString(primaryExpr.scope:getVariableName(primaryExpr.id))), {tmpReg}, {}, false);
                    self:addStatement(Ast.AssignmentStatement({Ast.AssignmentIndexing(self:env(scope), self:register(scope, tmpReg))},
...
function Compiler:compileExpression(expression, funcDepth, numReturns)
        ...
        for i=1, numReturns do
            if(i == 1) then
                if(expression.scope.isGlobal) then
                    ...
                    local tmpReg = self:allocRegister(false);
                    self:addStatement(self:setRegister(scope, tmpReg, self:obfuscateString(expression.scope:getVariableName(expression.id))), {tmpReg}, {}, false);
    ...
    if(expression.kind == AstKind.PassSelfFunctionCallExpression) then
        ...
        if(returnAll or numReturns > 1) then
            ...
            self:addStatement(self:setRegister(scope, tmpReg, self:obfuscateString(expression.passSelfFunctionName)), {tmpReg}, {}, false);
    ...
    if(self.BIN_OPS[expression.kind]) then
        ...
        for i=1, numReturns do
            ...
            if(i == 1) then
                ...
                scope:addReferenceToHigherScope(self.scope, self.vmHandlerVar);
                self:addStatement(self:setRegister(scope, regs[i], Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.vmHandlerVar), {
                    Ast.NumberExpression(self.opMapping[expression.kind]),
                    self:register(scope, lhsReg), self:register(scope, rhsReg)
