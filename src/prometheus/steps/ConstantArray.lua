...
function ConstantArray:createArray()
	...
	for i, v in ipairs(self.constants) do
		if type(v) == "string" then
			local encoded = self:encode(v);
			local bytes = {};
			for j = 1, #encoded do
				table.insert(bytes, Ast.TableEntry(Ast.NumberExpression(string.byte(encoded, j))));
			end
			entries[i] = Ast.TableEntry(Ast.TableConstructorExpression(bytes));
		else
...
function ConstantArray:addDecodeCode(ast)
	if self.Encoding == "base91" then
		local base91DecodeCode = [[
	do ]] .. table.concat(util.shuffle{
		...
		setmetatable(arr, {
			__index = function(t, k)
                ...
                if v + 1 > 0 then
                    insert(out, strchar((b + v * (2 ^ n)) % 256))
                end
                return concat(out)
			end
		})
		...
		local newAst = parser:parse(base91DecodeCode);
		local forStat = newAst.body.statements[1];
		forStat.body.scope:setParent(ast.body.scope);
		...
		visitast(newAst, nil, function(node, data)
			if(node.kind == AstKind.VariableExpression) then
				...
				if(node.scope:getVariableName(node.id) == "LOOKUP_TABLE") then
					data.scope:removeReferenceToHigherScope(node.scope, node.id);
					return self:createBase91Lookup();
				end
			end
		end)
...
function ConstantArray:createBase91Lookup()
	local entries = {};
	local i = 0;
	for char in string.gmatch(self.base91chars, ".") do
		table.insert(entries, Ast.KeyedTableEntry(Ast.NumberExpression(string.byte(char)), Ast.NumberExpression(i)));
		i = i + 1;
	end
	...
	if self.Encoding == "base91" then
        ...
        for i = 1, #str do
            ...
            if n > 13 then
                ...
                table.insert(out, b91:sub(v % 91 + 1, v % 91 + 1) .. b91:sub(math.floor(v / 91) + 1, math.floor(v / 91) + 1))
            end
        end
        if n > 0 then
            table.insert(out, b91:sub(b % 91 + 1, b % 91 + 1))
            if n > 7 or b > 90 then
                table.insert(out, b91:sub(math.floor(b / 91) + 1, math.floor(b / 91) + 1))
            end
        end
...
function ConstantArray:apply(ast, pipeline)
    ...
    local chars = {}
    for i = 33, 126 do
        if i ~= 34 and i ~= 39 and i ~= 92 then
            table.insert(chars, string.char(i))
        end
    end
	self.base91chars = table.concat(util.shuffle(chars));
