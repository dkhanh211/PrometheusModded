...
function Unparser:unparseExpression(expression, tabbing)
	...
	if(expression.kind == AstKind.NumberExpression) then
		if math.floor(expression.value) == expression.value and math.abs(expression.value) ~= math.huge then
			if expression.value < 0 then
				return string.format("-0x%X", -expression.value);
			else
				return string.format("0x%X", expression.value);
			end
		end
